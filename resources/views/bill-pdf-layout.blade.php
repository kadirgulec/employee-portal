...
<main class="mx-auto bg-white w-[210mm] h-[297mm] text-sm" id="bill">
    ...
    <div class="border-2 border-black -mt-0.5 p-1">
        ...
        <p class="-mt-0.5">Die Kosten für eine Stunde Support liegen bei 105,00 € zzgl. MwSt. innerhalb der Öffnungszeiten.</p>
        <p class="-mt-0.5">Die Supportzeiten werden, wenn nicht anders vereinbart, nach tatsächlichem Aufwand berechnet.</p>
        <p class="-mt-0.5">Die Supportzeiten werden, wenn nicht anders vereinbart, nach tatsächlichem Aufwand berechnet.</p>
        <p class="-mt-0.5">Die Supportzeiten werden, wenn nicht anders vereinbart, nach tatsächlichem Aufwand berechnet.</p>
        <p class="-mt-0.5">Die Supportzeiten werden, wenn nicht anders vereinbart, nach tatsächlichem Aufwand berechnet.</p>
    </div>
    ...
    <div class="border-2 border-black -mt-0.5 p-1">
        @foreach($bill->positions as $position)
            <h3 class="font-semibold text-base mt-1">{{$loop->iteration}} - {{$position->product_name}} <span class="text-sm font-normal">({{$position->product_price}}€ x {{$position->quantity}} = {{$position->product_price * $position->quantity}}€)</span></h3>
            <div class="[&>ul>li]:list-disc [&>ul>li]:list-inside [&>h3]:font-medium [&>*]:px-2">
    ...
    <div class="mt-2 pl-2">
        <p class="text-lg">Reparaturkostenfreigabe: <span class="font-bold"> {{number_format($bill->total_price,2,',')}} € zuzüglich MwSt.</span></p>
    ...
    <div class="border-2 border-black mt-2">
        ...
        <div class="border-t-2 border-black pl-2">
            <p class="font-semibold">Hiermit verpflichte ich mich, Kosten, die durch Leistungen der aks Service GmbH entstehen, nach den genannten Bedingungen zu akzeptieren.</p>
